  ...
  <div class="gl-report-page">
    ...
    <header class="page-header">
      <div>
        ...
        <p class="muted">A complete chronological ledger of customer invoices, payment receipts, and receivables.</p>
      </div>
    ...
    <!-- Sub-navigation Tabs -->
    <ReportHeaderTabs activeTab="customer" />
    ...
    <section class="filters-card no-print">
      <div class="filter-row">
        ...
        <label class="search-label">
          <span>Search</span>
          <input type="search" v-model="filters.search" placeholder="Search customer, invoice, ref, receipt..." @input="loadReport">
        </label>
    ...
    <div class="print-header">
      ...
      <p v-if="filters.customer_id">Customer: {{ selectedCustomerName }}</p>
    </div>

    <!-- Summary Cards -->
    <section v-if="report" class="summary-cards">
      <div class="summary-card opening">
        <span class="card-label">Opening Customer Balance</span>
        <strong class="card-value" :class="{ negative: report.opening_balance < 0 }">
          {{ money(convertSummary(report.opening_balance)) }}
        </strong>
        <span class="card-sub">Receivable brought forward</span>
      </div>
      <div class="summary-card credits">
        <span class="card-label">Total Invoiced (Debits)</span>
        <strong class="card-value positive">+ {{ money(convertSummary(report.total_debits)) }}</strong>
        <span class="card-sub">Billed to customer</span>
      </div>
      <div class="summary-card debits">
        <span class="card-label">Total Received (Credits)</span>
        <strong class="card-value negative">- {{ money(convertSummary(report.total_credits)) }}</strong>
        <span class="card-sub">Payments collected</span>
      </div>
      <div class="summary-card closing">
        <span class="card-label">Closing Customer Balance</span>
        <strong class="card-value" :class="report.closing_balance >= 0 ? 'positive' : 'negative'">
          {{ money(convertSummary(report.closing_balance)) }}
        </strong>
        <span class="card-sub">Net receivable balance</span>
      </div>
    </section>
    ...
    <div v-if="loading" class="state-box">
      ...
      <p>Generating customer ledger entries...</p>
    </div>

    <!-- Empty State -->
    <div v-else-if="report && report.entries.length === 0" class="state-box">
      <svg viewBox="0 0 24 24" width="48" height="48" style="color:var(--muted)"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" fill="currentColor"/></svg>
      <p>No customer transactions or invoices found for the selected period.</p>
    </div>
      ...
      <table class="gl-table">
        <thead>
          <tr>
            ...
            <th class="right" style="width:140px">Debit (Invoice)</th>
            <th class="right" style="width:140px">Credit (Receipt)</th>
        ...
        <tbody>
          ...
          <tr v-for="(entry, idx) in report.entries" :key="entry.id" :class="rowClass(entry)">
            ...
            <td class="ref-cell">
              <RouterLink
                v-if="entry.entry_type === 'Invoice' && entry.invoice_id"
                :to="`/invoices/${entry.invoice_id}`"
                class="ref-link"
                title="View Invoice Details"
              >
                <span class="ref-badge">{{ entry.reference || entry.invoice_number }}</span>
              </RouterLink>
              <RouterLink
                v-else-if="entry.transaction_id || entry.id"
              ...
              <span v-else class="ref-badge">{{ entry.reference || '—' }}</span>
            </td>
            ...
            <td>
              <div class="entry-desc">
                <strong>{{ entry.transaction_type || entry.category || 'Customer Movement' }}</strong>
                <p v-if="entry.description" class="desc-text">{{ entry.description }}</p>
                <div v-if="entry.product_name || entry.project_name" class="tax-tags">
                  <span v-if="entry.product_name">Product: {{ entry.product_code ? `${entry.product_code} · ` : '' }}{{ entry.product_name }}</span>
            ...
            <td class="mono">
              <RouterLink
                v-if="entry.invoice_id && entry.invoice_number && entry.invoice_number !== '—'"
                :to="`/invoices/${entry.invoice_id}`"
                class="entity-link"
              >
                {{ entry.invoice_number }}
              </RouterLink>
              <span v-else>{{ entry.invoice_number || '—' }}</span>
            </td>
            <td class="mono">
              <RouterLink
                v-if="entry.receipt_number && entry.receipt_number !== '—'"
                :to="`/finance/transactions/${entry.receipt_number}`"
                class="entity-link"
              >
                {{ entry.receipt_number }}
              </RouterLink>
              <span v-else>{{ entry.receipt_number || '—' }}</span>
            </td>
            <td class="text-cell">{{ entry.created_by_name || '—' }}</td>
            <td>
              <span class="status-badge" :class="{ reversed: entry.status === 'Reversed' || entry.status === 'Cancelled' }">
                {{ entry.status || 'Active' }}
              </span>
            </td>
          ...
          <tr class="closing-row">
            <td colspan="9"><strong>Closing Balance</strong> — Net Balance Owed</td>
            <td class="right mono positive"><strong>+ {{ money(convertSummary(report.total_debits)) }}</strong></td>
            <td class="right mono negative"><strong>- {{ money(convertSummary(report.total_credits)) }}</strong></td>
...
const selectedCustomerName = computed(() => {
  ...
  const c = customers.value.find(item => String(item.id) === String(filters.customer_id))
  return c ? (c.company_name || c.name) : `Customer #${filters.customer_id}`
})

// ── Lifecycle ──────────────────────────────────────────────────────
onMounted(async () => {
  try {
    const custData = await apiGet('/api/customers')
...
async function loadReport() {
  ...
  try {
    ...
    const data = await apiGet(`/api/finance/reports/customer-ledger?${params}`)
    report.value = data
  } catch (e) {
    console.error('Failed to load Customer Ledger report:', e)
  } finally {
    loading.value = false
  }
}
...
function rowClass(entry) {
  if (entry.status === 'Reversed' || entry.status === 'Cancelled') return 'reversed-row'
  return entry.entry_type === 'Invoice' ? 'credit-row' : 'debit-row'
}
...
function exportCSV() {
  ...
  const headers = [
    'Date', 'Ref ID', 'Customer Name', 'Category', 'Description', 
    'Invoice Number', 'Receipt Number', 'Created By', 'Status', 'Debit (Invoice)', 'Credit (Receipt)', 'Running Balance'
  ]
  ...
  link.setAttribute('download', `Customer_Ledger_${filters.start_date || 'All'}_to_${filters.end_date || 'All'}.csv`)
...
function exportExcel() {
  ...
  const link = document.createElement('a')
  link.setAttribute('href', url)
  link.setAttribute('download', `Customer_Ledger_${filters.start_date || 'All'}_to_${filters.end_date || 'All'}.xls`)
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
}
